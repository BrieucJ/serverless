...
export default (response: GraphQLResponse): GraphQLResponse => {
  ...
  if (response.errors) {
    response.data = null;
    response.errors = response.errors.reduce(
      (
        acc: GraphQLFormattedError[],
        ...
        if (error.message && error.message === "VALIDATION_ERROR") {
          ...
          const extensionsErrors = error.extensions
            ?.errors as ValidationError[];
          const errors = extensionsErrors.reduce(
            (
              acc2: GraphQLFormattedError[],
              err: ValidationError
            ): GraphQLFormattedError[] => {
